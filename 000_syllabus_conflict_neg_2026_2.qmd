...
params:
  ...
  schedule:
    value:
      meeting_dates: [3, 5]
      dates_recess: [2026-10-05, 2026-10-11]
      start_presentation_hour: 19:00
      end_presentation_hour: 20:00  
...
-   Transversal learning outcomes
    ...
    - Analyze situations in academic, social, or professional contexts by applying ethical and civic principles to justify sound decisions.   
    - Apply knowledge and experience in real or simulated scenarios to make strategic decisions and enhance performance.
...
tibble(
  Week = c("1", "2", "3", "3", "4", "5", "6", "6", "7", "8", "9", "9"),
  Material = rep(c(lewicki, "[@program_on_negotiation_three_2008]"), times = 3),
  `Chapter or Round` = c(
    "1",
    "2",
    "3",
...
-   `r format(x = ymd(params$block$start_date), format = "%B %e")`
    ...
    -   **Virtual meeting Presentation Syllabus**
        ...
        -   **Prerequisites**:
            -   Review the course program in advance, which can be downloaded by following the route *Primer corte 30% \> Contenido y actividades complementarias \> Support material \> Presentations \> 000_syllabus_conflict_neg\_...*
...
-   `r format(x = virtual_meeting[[4]], format = "%B %e")`
    ...
    -   **Virtual meeting Workshop: Resolviendo conflictos**
        -   **Meeting time**: `r paste(params$schedule$start_presentation_hour, params$schedule$end_presentation_hour, sep = " - ")` hours
        -   **Prerequisites**:
            -   Read and review [@lewicki_negociacion_2024, Chapter 2]
            -   Have a headset with a built-in microphone to isolate noise from the environment
...
-   `r format(x = virtual_meeting[[10]], format = "%B %e")`
    ...
    -   **Virtual meeting Workshop: Gestión de Emociones Inicial**
        -   **Meeting time**: `r paste(params$schedule$start_presentation_hour, params$schedule$end_presentation_hour, sep = " - ")` hours
        -   **Prerequisites**:
            -   Read and review [@lewicki_negociacion_2024, Chapter 6]
            -   Have a headset with a built-in microphone to isolate noise from the environment
...
-   `r format(x = virtual_meeting[[16]], format = "%B %e")`
    ...
    -   **Virtual meeting Workshop: Habla y Escucha: Comunicación Asertiva**
        -   **Meeting time**: `r paste(params$schedule$start_presentation_hour, params$schedule$end_presentation_hour, sep = " - ")` hours
        -   **Prerequisites**:
            -   Read and review [@lewicki_negociacion_2024, Chapter 5]
            -   Have a headset with a built-in microphone to isolate noise from the environment
