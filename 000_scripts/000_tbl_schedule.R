...
tbl_schedule <- function(
  ...
  # Base tibble
  block_dates <- tibble::tibble(
    index = timetk::tk_make_timeseries(
  ...
  if (is.null(dates_recess)) {
    ...
    block_dates <- block_dates |>
      dplyr::mutate(
        virtual_meeting = dplyr::if_else(
          condition = day %in% {{ meeting_days }},
          true = 1,
          false = 0
        )
      )
    ...
    block_dates <- block_dates |>
      dplyr::mutate(
        activity = dplyr::if_else(
          condition = (((day == {{ open_day_activity }}) &
    ...
    block_dates <- block_dates |>
      dplyr::mutate(
        recess = dplyr::if_else(
          condition = timetk::between_time(
            index = index,
    ...
    block_dates <- block_dates |>
      dplyr::mutate(block_week = lubridate::isoweek(index)) |>
      dplyr::mutate(
        block_week = dplyr::if_else(
          index >= lubridate::ymd(dates_recess[[1]]),
          true = block_week - 1,
          false = block_week
        )
      ) |>
...
tbl_schedule(
  ...
  end_date = "2026-11-28",
  dates_recess = c("2026-10-05", "2026-10-10")
)
