...
-   This presentation is based on [@lewicki_negociacion_2024, Chapter 3]

##

- **Purpose**

    - Understand the fundamental principles of integrative negotiation, along with the strategies and tactics used to create mutual value.
...
ggplot(
  data = tibble(x = c(1, -1, -1, 0, 0, 4, 2.4), y = c(-1, 1, -1, 0, 4, 0, 2.4))
) +
  geom_point(
    aes(x = x, y = y),
    fill = umng_palette[2],
  ...
  # red curve
  geom_curve(
    aes(x = 0, y = 4, xend = 4, yend = 0),
    color = umng_palette[4],
    curvature = -0.2
  ) +
  # x-axis
  geom_segment(
    aes(x = 0, y = -2, xend = 0, yend = 4.5),
    arrow = arrow(type = "closed")
  ) +
  # y-axis
  geom_segment(
    aes(x = -2, y = 0, xend = 4.5, yend = 0),
    arrow = arrow(type = "closed")
  ) +
  # green-arrow
  geom_segment(
    aes(x = 0, y = 0, xend = 2.3, yend = 2.3),
    arrow = arrow(type = "closed"),
