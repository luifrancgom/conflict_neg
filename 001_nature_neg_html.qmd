...
- **Purpose**
    ...
    - Understand the definition of negotiation, key components of the negotiation process, major types of negotiation identified in the literature, and the relationship between negotiation and conflict management.

# The roles managers play, negotiation and conflict resolution

##

![Henry Mintzberg (1939 - ?)](000_images/001_henry_mintzberg.jpg){.nostretch #fig-henry-mintzberg width=40%}
...
# Organizational Behavior, negotiation and conflict

##

```{mermaid}
%%| label: fig-bo-model
%%| fig-cap: Basic Organizational Behavior Model [@robbins_organizational_2024, p. 61]
%%| fig-height: 5
...
flowchart LR
    ...
    classDef Individual fill:#fdc600,stroke:#043074,stroke-width:1px;
    classDef Group fill:#00b3f0,stroke:#043074,stroke-width:1px;
    classDef Organizational fill:#16a085,stroke:#043074,stroke-width:1px;

    class A1,B1,C1 Individual;
    class A2,B2,C2 Group;
    class A3,B3,C3 Organizational;
    
    style S1 fill:none,stroke:#043074,stroke-width:2px
    style S2 fill:none,stroke:#043074,stroke-width:2px
    style S3 fill:none,stroke:#043074,stroke-width:2px
```
...
conflic_diag <- tibble(
  ...
  mutate(
    Level = factor(Level, levels = c("Easy to resolve", "Difficult to resolve"))
  )
...
conflic_diag |>
  ggplot() +
  geom_segment(
    data = conflic_diag_level,
    aes(x = Level_easy, xend = Level_difficult, y = Dimension, yend = Dimension)
  ) +
  geom_label(
    aes(x = Level, y = Dimension, label = label_text, fill = Dimension),
    show.legend = FALSE,
    fontface = "bold",
    size = 3,
    nudge_y = 0.4,
  ...
  geom_point(
    aes(x = Level, y = Dimension, fill = Level),
    size = 5,
    shape = 21,
    color = "black"
  ) +
...
dual_concern_model |>
  ggplot() +
  geom_label(aes(x = x, y = y, label = labels), size = 5, fill = "#fdc600") +
  geom_segment(
    data = tibble(x = 0, y = 0, yend = 0, xend = 6),
    aes(x = x, y = y, xend = xend, yend = yend),
    color = "#043074"
  ) +
  geom_segment(
    data = tibble(x = 0, y = 0, yend = 6, xend = 0),
    aes(x = 0, y = 0, yend = 6, xend = 0),
    color = "#043074"
  ) +
  scale_x_continuous(limits = c(0, 6)) +
  labs(x = "Concern about own outcomes", y = "Concern about other’s outcomes") +
  theme(
    panel.border = element_rect(fill = NA, color = "black"),
    plot.background = element_rect(fill = "white"),
    panel.background = element_rect(fill = "white"),
